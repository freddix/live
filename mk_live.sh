...
RPM_CORE_LIST=$(cat ${LIVECD_DIR}/config/rpm-core.list)
RPM_BASE_LIST=$(cat ${LIVECD_DIR}/config/rpm-base.list)
...
#RPM_DE_LIST=$(cat ${LIVECD_DIR}/config/rpm-custom.list)
...
create_system() {
  ...
  LC_ALL=C poldek -vv --upa --conf "${POLDEK_DIR}/poldek_$KERNEL_ARCH.conf"
  LC_ALL=C poldek --root="${FS_DIR}" --conf "${POLDEK_DIR}/poldek_$KERNEL_ARCH.conf" \
    -u ${RPM_CORE_LIST} ${RPM_BASE_LIST} ${RPM_X_LIST} ${RPM_DE_LIST} \
  ...
  # add live user
  chroot ${FS_DIR} /usr/sbin/useradd -m -G adm,wheel,audio,video,cdrom,systemd-journal -s /usr/bin/zsh -c Freddix live
  ...
  if [ "$RPM_X_LIST" == "" ]; then
    ...
    echo "NO_AT_BRIDGE=1" >> ${FS_DIR}/etc/environment
    ...
    if [ -f ${FS_DIR}/etc/lightdm/lightdm.conf ]; then
      ...
      sed -i -e 's|#autologin-user=.*|autologin-user=live|' \
        -i -e 's|#user-session=.*|user-session=mate|' \
        ${FS_DIR}/etc/lightdm/lightdm.conf
    fi
  ...
  rm -f  ${FS_DIR}/etc/X11/xorg.conf.d/20-video-intel.conf
  rm -f ${FS_DIR}/usr/lib/systemd/system/sysinit.target.wants/systemd-udev-hwdb-update.service
  cp -a ${LIVECD_DIR}/config/poldek/repos.d/* ${FS_DIR}/etc/poldek/repos.d

  # basic networking
  cp ${LIVECD_DIR}/config/dhcp.network \
    ${FS_DIR}/etc/systemd/network
  chroot ${FS_DIR} systemctl enable systemd-resolved
  ln -sf  /run/systemd/resolve/resolv.conf \
    ${FS_DIR}/etc/resolv.conf

  # redirect journal to tty12
  echo "ForwardToConsole=yes\nTTYPath=/dev/tty12\nMaxLevelConsole=info\n" >> ${FS_DIR}/etc/systemd/journald.conf

  mkdir ${FS_DIR}/home/users/live/.ssh
  cp config/authorized_keys ${FS_DIR}/home/users/live/.ssh

  cat ${LIVECD_DIR}/config/locale >> ${FS_DIR}/etc/localedb.gen
...
create_boot() {
  ...
  misc_modules="dm-snapshot loop nls_cp437 crc32c crc32c-intel"
  ...
  LC_ALL=C poldek --root="${INITRD_DIR}" --conf "${POLDEK_DIR}/poldek_$KERNEL_ARCH.conf" \
    -i device-mapper busybox-live kmod libsmartcols
  ...
  for applet in $(chroot ${INITRD_DIR} busybox --list); do
...
create_fs () {
  ...
  # LZ4: -comp lz4 -Xhc
  /usr/sbin/mksquashfs ${ext_image} ${IMAGE_DIR}/freddix.sfs -noappend -no-progress -comp lzo # xz -Xbcj x86 -b 1048576 -Xdict-size 1048576
}
